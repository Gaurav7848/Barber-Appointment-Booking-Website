...
import { Calendar, Clock, Star, MapPin, User, Settings, LogOut, Heart, MessageSquare, Bell, BellOff, TrendingUp, XCircle } from 'lucide-react'
...
const CustomerDashboard = () => {
  ...
  const monthlyData = appointments.reduce((acc, a) => {
    const month = new Date(a.date).toLocaleDateString('en-IN', { month: 'short' })
    acc[month] = (acc[month] || 0) + (a.amount || 0)
    return acc
  }, {})
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-4 max-w-2xl">
              <div>
                ...
                <input type="text" defaultValue={user?.name} className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50" disabled />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="tel" defaultValue={user?.phone} className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50" disabled />
              </div>
        ...
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <StatCard title="Upcoming" value={upcomingAppointments.length} icon={Calendar} color="#2563eb" bgColor="#dbeafe" />
              <StatCard title="Past Appointments" value={pastAppointments.length} icon={Clock} color="#059669" bgColor="#d1fae5" />
              <StatCard title="Cancelled" value={cancelledAppointments.length} icon={XCircle} color="#dc2626" bgColor="#fee2e2" />
            </div>
            {chartData.length > 0 && (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
              ...
              <div className="space-y-4">
                {displayAppointments.map((appointment) => (
                  <div key={appointment._id} className="border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow">
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-dark to-gray-800 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          ...
          <button onClick={() => { localStorage.removeItem('token'); localStorage.removeItem('user'); navigate('/') }} className="flex items-center gap-2 bg-red-600 px-4 py-2 rounded-lg hover:bg-red-700 transition-colors">
                ...
                {menuItems.map((item) => (
                  <button
                    ...
                    className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-left transition-colors ${
                      activeTab === item.id ? 'bg-primary text-white shadow-md' : 'hover:bg-gray-100 text-gray-700'
                    }`}
                  >
